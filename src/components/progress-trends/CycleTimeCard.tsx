    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <button
                    type="button"
                    className="inline-flex items-center text-muted-foreground hover:text-foreground/80"
                    aria-label="설명 보기"
                >
                    <HelpCircle className="h-3.5 w-3.5" />
                </button>
            </PopoverTrigger>
            <PopoverContent className="w-80 p-3 text-xs text-foreground/90 bg-card border border-border" align="start">
                {children}
            </PopoverContent>
        </Popover>
    );
            ...
            <div className="flex items-center justify-between mb-2">
                <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
                    ...
                    <HelpTooltip>
                        ...
                        <p className="mt-2 text-muted-foreground">type별로 비교하면 "결함은 평균 1일, 새 기능은 5일" 같은 패턴 인식 가능.</p>
                    </HelpTooltip>
                </h3>
                {isFetching && <span className="text-[11px] text-muted-foreground">분석 중...</span>}
            </div>

            {!stats || stats.length === 0 ? (
                <div className="text-sm text-muted-foreground py-4 flex items-start gap-2">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-muted/40 border-b border-border">
                                <tr>
                                    <th scope="col" className="px-2 py-2 text-xs font-medium text-foreground/80 text-left">타입</th>
                                    <th scope="col" className="px-2 py-2 text-xs font-medium text-foreground/80 text-right">표본</th>
                                    <th scope="col" className="px-2 py-2 text-xs font-medium text-foreground/80 text-right">Active 평균</th>
                                    <th scope="col" className="px-2 py-2 text-xs font-medium text-foreground/80 text-right">Active P85</th>
                                    <th scope="col" className="px-2 py-2 text-xs font-medium text-foreground/80 text-right">Lead 평균</th>
                                    <th scope="col" className="px-2 py-2 text-xs font-medium text-foreground/80 text-right" title="Wait/Lead — 블로커·리뷰 대기 비중">대기 비중</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border/50">
                                {stats.map((s) => (
                                    <tr key={s.type} className="hover:bg-muted/40">
                                        <td className="px-2 py-1.5 text-foreground">{s.type}</td>
                                        <td className="px-2 py-1.5 text-right tabular-nums text-foreground/80">{s.sampleSize}건</td>
